...
class ExpertDecoder(AutoregressiveDecoder):
    ...
    def decoding_w(self) -> tf.Variable:
        # We only define decoding_w because of its requirement
        # in the AutoregressiveDecoder.decoding_loop
        return tf.concat([d.decoding_w for d in self.decoders], -1)

    @tensor
    def decoding_b(self) -> Optional[tf.Variable]:
        # see decoding_w
        return tf.concat([d.decoding_b for d in self.decoders], -1)
